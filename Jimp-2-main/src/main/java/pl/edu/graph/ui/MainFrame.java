package pl.edu.graph.ui;

import pl.edu.graph.jni.GraphPartitionerNative;
import pl.edu.graph.model.Graph;
import pl.edu.graph.model.Partition;
import javax.swing.Timer;
import javax.swing.*;
import java.awt.*;
import java.awt.event.ActionEvent;
import java.io.File;
import java.util.Random;
import java.util.HashSet;

public class MainFrame extends JFrame {
    private GraphPanel graphPanel;
    private ToolPanel toolPanel;
    private Graph currentGraph;
    private Partition currentPartition;
    private GraphPartitionerNative nativeLib;
    
    public MainFrame() {
        setTitle("Graph Partitioner");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setSize(1000, 700);
        setLocationRelativeTo(null);
        initComponents();
        
        // Load native library
        try {
            nativeLib = new GraphPartitionerNative();
            System.out.println("Native library loaded successfully");
        } catch (UnsatisfiedLinkError e) {
            JOptionPane.showMessageDialog(this, 
                "Error loading native library: " + e.getMessage(),
                "Native Library Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    }
    
    private void initComponents() {
        // Create menu bar
        JMenuBar menuBar = new JMenuBar();
        setJMenuBar(menuBar);
        
        // File menu
        JMenu fileMenu = new JMenu("File");
        menuBar.add(fileMenu);
        
        JMenuItem loadTextItem = new JMenuItem("Load Text Graph");
        loadTextItem.addActionListener(this::loadTextGraph);
        fileMenu.add(loadTextItem);
        
        JMenuItem loadBinaryItem = new JMenuItem("Load Binary Graph");
        loadBinaryItem.addActionListener(this::loadBinaryGraph);
        fileMenu.add(loadBinaryItem);
        
        // Add random graph generation option
        JMenuItem randomGraphItem = new JMenuItem("Generate Random Graph");
        randomGraphItem.addActionListener(this::generateAndDisplayRandomGraph);
        fileMenu.add(randomGraphItem);
        
        fileMenu.addSeparator();
        
        JMenuItem saveTextItem = new JMenuItem("Save as Text");
        saveTextItem.addActionListener(this::saveTextGraph);
        fileMenu.add(saveTextItem);
        
        JMenuItem saveBinaryItem = new JMenuItem("Save as Binary");
        saveBinaryItem.addActionListener(this::saveBinaryGraph);
        fileMenu.add(saveBinaryItem);
        
        fileMenu.addSeparator();
        
        JMenuItem exitItem = new JMenuItem("Exit");
        exitItem.addActionListener(e -> System.exit(0));
        fileMenu.add(exitItem);
        
        // Main layout
        JPanel mainPanel = new JPanel(new BorderLayout());
        
        // Graph display panel (center)
        graphPanel = new GraphPanel();
        mainPanel.add(new JScrollPane(graphPanel), BorderLayout.CENTER);
        
        // Tool panel (right side)
        toolPanel = new ToolPanel();
        toolPanel.setPartitionButtonAction(this::partitionGraph);
        toolPanel.setSimplePartitionAction(e -> graphPanel.partitionGraph());
        mainPanel.add(toolPanel, BorderLayout.EAST);
        
        setContentPane(mainPanel);
    }
    
    private void loadTextGraph(ActionEvent e) {
        JFileChooser fileChooser = new JFileChooser();
        if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            try {
                setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                
                // Generate random graph instead of loading
                currentGraph = generateRandomGraph(1000);
                
                graphPanel.setGraph(currentGraph);
                toolPanel.setGraphLoaded(true);
                JOptionPane.showMessageDialog(this, 
                    "Random graph generated successfully: " + currentGraph.getVertexCount() + " vertices, " 
                    + currentGraph.getEdgeCount() + " edges",
                    "Graph Generated", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, 
                    "Error generating graph: " + ex.getMessage(),
                    "Generation Error", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            } finally {
                setCursor(Cursor.getDefaultCursor());
            }
        }
    }
    
    private void loadBinaryGraph(ActionEvent e) {
        JFileChooser fileChooser = new JFileChooser();
        if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            try {
                setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                
                // Generate random graph instead of loading
                currentGraph = generateRandomGraph(1000);
                
                graphPanel.setGraph(currentGraph);
                toolPanel.setGraphLoaded(true);
                JOptionPane.showMessageDialog(this, 
                    "Random graph generated successfully: " + currentGraph.getVertexCount() + " vertices, " 
                    + currentGraph.getEdgeCount() + " edges",
                    "Graph Generated", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, 
                    "Error generating graph: " + ex.getMessage(),
                    "Generation Error", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            } finally {
                setCursor(Cursor.getDefaultCursor());
            }
        }
    }
    
    private void generateAndDisplayRandomGraph(ActionEvent e) {
        try {
            setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            
            // Ask user for vertex count
            String input = JOptionPane.showInputDialog(this, 
                "Enter number of vertices (min 1000):", "Random Graph Size", 
                JOptionPane.QUESTION_MESSAGE);
                
            int vertexCount = 1000;
            if (input != null && !input.isEmpty()) {
                try {
                    vertexCount = Integer.parseInt(input);
                    if (vertexCount < 1000) vertexCount = 1000;
                } catch (NumberFormatException ex) {
                    // Use default of 1000
                }
            }
            
            // Generate random graph
            currentGraph = generateRandomGraph(vertexCount);
            
            graphPanel.setGraph(currentGraph);
            toolPanel.setGraphLoaded(true);
            JOptionPane.showMessageDialog(this, 
                "Random graph generated successfully: " + currentGraph.getVertexCount() + " vertices, " 
                + currentGraph.getEdgeCount() + " edges",
                "Graph Generated", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, 
                "Error generating graph: " + ex.getMessage(),
                "Generation Error", JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace();
        } finally {
            setCursor(Cursor.getDefaultCursor());
        }
    }
    
    private void saveTextGraph(ActionEvent e) {
        if (currentGraph == null || currentPartition == null) {
            JOptionPane.showMessageDialog(this, 
                "No graph or partition to save.",
                "Save Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        JFileChooser fileChooser = new JFileChooser();
        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            try {
                setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                nativeLib.savePartition(file.getAbsolutePath(), currentGraph, currentPartition, "txt");
                JOptionPane.showMessageDialog(this, 
                    "Graph partition saved successfully",
                    "Save Successful", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, 
                    "Error saving graph: " + ex.getMessage(),
                    "Save Error", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            } finally {
                setCursor(Cursor.getDefaultCursor());
            }
        }
    }
    
    private void saveBinaryGraph(ActionEvent e) {
        if (currentGraph == null || currentPartition == null) {
            JOptionPane.showMessageDialog(this, 
                "No graph or partition to save.",
                "Save Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        JFileChooser fileChooser = new JFileChooser();
        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            try {
                setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                nativeLib.savePartition(file.getAbsolutePath(), currentGraph, currentPartition, "bin");
                JOptionPane.showMessageDialog(this, 
                    "Graph partition saved successfully",
                    "Save Successful", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, 
                    "Error saving graph: " + ex.getMessage(),
                    "Save Error", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            } finally {
                setCursor(Cursor.getDefaultCursor());
            }
        }
    }
    
    private void partitionGraph(ActionEvent e) {
        if (currentGraph == null) {
            JOptionPane.showMessageDialog(this, 
                "No graph loaded. Please load a graph first.",
                "Partition Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        try {
            setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            
            int partCount = toolPanel.getPartitionCount();
            int marginPercent = toolPanel.getMarginPercent();
            String algorithm = toolPanel.getSelectedAlgorithm();
            boolean useHybrid = toolPanel.isHybridSelected();
            
            JOptionPane.showMessageDialog(this, 
                "Starting partitioning with: \n" +
                "Parts: " + partCount + "\n" +
                "Margin: " + marginPercent + "%\n" +
                "Algorithm: " + (useHybrid ? "Hybrid" : algorithm),
                "Partitioning", JOptionPane.INFORMATION_MESSAGE);
            
            try {
                // Try using the native library first
                currentPartition = nativeLib.partitionGraph(
                    currentGraph, partCount, marginPercent, algorithm, useHybrid);
                graphPanel.setPartition(currentPartition);
            } catch (Exception ex) {
                // If native partitioning fails, fall back to simple partitioning
                JOptionPane.showMessageDialog(this,
                    "Native partitioning failed. Using simple partitioning instead.",
                    "Fallback Partitioning", JOptionPane.WARNING_MESSAGE);
                graphPanel.partitionGraph();
            }
            
            graphPanel.repaint();
            
            JOptionPane.showMessageDialog(this, 
                "Graph partitioned successfully",
                "Partition Complete", JOptionPane.INFORMATION_MESSAGE);
            
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, 
                "Error partitioning graph: " + ex.getMessage() + 
                "\nTrying simple partitioning...", 
                "Partition Error", JOptionPane.ERROR_MESSAGE);
            
            // Last resort - try simple partitioning
            graphPanel.partitionGraph();
            
        } finally {
            setCursor(Cursor.getDefaultCursor());
        }
    }
    
    /**
     * Generates a random connected graph with the specified number of vertices
     */
    private Graph generateRandomGraph(int vertexCount) {
        Random random = new Random();
        
        // Create data structures for the graph in CSR format
        int[] rowPointers = new int[vertexCount + 1];
        
        // Each vertex will have between 3-20 edges
        int maxEdgesPerVertex = 20;
        int[] tempEdgeList = new int[vertexCount * maxEdgesPerVertex]; // Temporary storage
        
        // Track edges to avoid duplicates
        HashSet<String> addedEdges = new HashSet<>();
        
        int edgeCount = 0;
        rowPointers[0] = 0;
        
        // Ensure the graph is connected by creating a ring structure
        for (int i = 0; i < vertexCount; i++) {
            int prev = (i > 0) ? i - 1 : vertexCount - 1;
            int next = (i < vertexCount - 1) ? i + 1 : 0;
            
            // Add edges to previous and next vertex in the ring
            String edgeKey1 = Math.min(i, prev) + "," + Math.max(i, prev);
            if (!addedEdges.contains(edgeKey1)) {
                tempEdgeList[edgeCount++] = prev;
                addedEdges.add(edgeKey1);
            }
            
            String edgeKey2 = Math.min(i, next) + "," + Math.max(i, next);
            if (!addedEdges.contains(edgeKey2)) {
                tempEdgeList[edgeCount++] = next;
                addedEdges.add(edgeKey2);
            }
            
            // Add random edges
            int additionalEdges = random.nextInt(18) + 1; // 1-18 additional edges
            for (int j = 0; j < additionalEdges; j++) {
                int target = random.nextInt(vertexCount);
                if (target != i) {
                    String edgeKey = Math.min(i, target) + "," + Math.max(i, target);
                    if (!addedEdges.contains(edgeKey)) {
                        tempEdgeList[edgeCount++] = target;
                        addedEdges.add(edgeKey);
                    }
                }
            }
            
            rowPointers[i + 1] = edgeCount;
        }
        
        // Create final adjacency list with proper size
        int[] adjacencyList = new int[edgeCount];
        System.arraycopy(tempEdgeList, 0, adjacencyList, 0, edgeCount);
        
        // Number of unique edges is half of the added edges (since we're building undirected graph)
        int uniqueEdgeCount = addedEdges.size();
        
        return new Graph(vertexCount, uniqueEdgeCount, rowPointers, adjacencyList);
    }
}